...
var homes =  plugin( 'homes', { 
  ...
  help: function( ) {
    return [
      ...
      '/jsp home set_outside : Set your current location as outside your home',
  ...
  go: function( guest, host ) {
    ...
    if (isOutside) {
      loc = _store.houses_outside[ host.name ];
    } else {
    ...
    if ( !(isOutside) && !(this._canVisit( guest, host )) ) {
  ...
  set_outside: function( player ) {
    player = utils.player( player );
    var loc = player.location;
    _store.houses_outside[player.name] = utils.locationToJSON( loc );
  },
...
var options = {
  ...
  'set_outside': function( params, sender ) { 
    homes.set_outside( sender ); 
  },
...
command( 'home', function ( params , sender) {
  ...
  } else if (params[0] === 'outside') {
    homes.go( sender, 'outside' );
  } else {
